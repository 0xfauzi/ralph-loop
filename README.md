...
`ks sense --write-baseline` records those measurements to `scripts/kstrl/sense-baseline.json` and `ks sense --compare-baseline` reports what a branch added to it - new failure signatures, signatures whose count rose, and signatures the branch fixed. That is the dampener: it stops concurrent work undoing the loop's progress while the loop runs. It is advisory (exit 0 either way) until you pass `--fail-on-regression`, and a workflow on this repository posts the report on every pull request. How to adopt it and how to graduate to blocking: [docs/dampener.md](docs/dampener.md).
