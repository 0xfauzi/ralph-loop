...
It is recorded as `failed_check = adversarial_budget` and journalled as `review:budget-exhausted` or `security:budget-exhausted`, with an `infrastructure_error` finding for the phase. It does not retry: the budget only shrinks, so a retry would burn engineer iterations against the same cap. `ks serve` reads that `failed_check` and classifies the whole run as `budget_halt`, which is terminal: the queue item is not requeued, because the cap starts again at zero on the next attempt and the run would stop at the same component.

Two consequences worth knowing before you set a cap. Every component that depends on a halted one is SKIPPED (`cascade_skip`), so one halt near the root of the DAG can end most of the run. And each halted component files an inbox item keyed `halted:<comp>:<phase>:adversarial_budget`; `[inbox] open_item_cap` (default 50) stops `ks serve` admitting new queue work once the inbox is that full, so a cap far below the run's need can quietly reach it.
